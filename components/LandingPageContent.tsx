...
import { LPConfig } from '@/lib/lp-configs';

type Props = {
  config: LPConfig;
};

export default function LandingPageContent({ config }: Props) {
  return (
    <>
      ...
      <main className="l-main">
        <div className="p-fv">
          <picture className="p-fv__img">
            <source media="(min-width: 768px)" srcSet={config.fv.pcImage} />
          ...
          <div className="p-fv__text-wrapper">
            <p className="p-fv__lead">
              <img src="/lp/img/fv-text.svg" alt={config.fv.leadText.line1} />で、<br />
              <span className="--margin">
                {config.fv.leadText.line2.split('').map((char, index) => (
                  <span key={index} className="p-fv__carrier-up">{char}</span>
                ))}
                <span className="--space">{config.fv.leadText.line3.charAt(0)}</span>
              </span>
              <br /><span className="--height">{config.fv.leadText.line3.slice(1)}</span>
            </p>
            <div 
              className="p-fv__text" 
              dangerouslySetInnerHTML={{ __html: config.fv.subText }}
            />
          </div>
          <div className="p-fv__box">
            <p 
              className="p-fv__message"
              dangerouslySetInnerHTML={{ __html: config.fv.message }}
            />
            <div className="c-button">
              <img src="/lp/img/icon-cta.svg" alt="簡単!30秒" />
              <DeviceAwareLink 
                mobileUrl={config.cta.mobileUrl}
                desktopUrl={config.cta.desktopUrl}
              >
        ...
        <section className="p-footer-cta">
          <div className="l-inner">
            <div className="p-footer-cta__contents">
              <p 
                className="p-footer-cta__text--sub"
                dangerouslySetInnerHTML={{ __html: config.fv.message.replace('<br />', '<br class="u-sp" />') }}
              />
